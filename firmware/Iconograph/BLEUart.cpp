...
static void nusSend(uint8_t *data, unsigned len);
...
//static char c = 'A';
    ...
    if (p_evt->type == BLE_NUS_EVT_RX_DATA && g_uartInstance != 0)
    {
        ...
        uint8_t response[16];
        strcpy((char *)response, "OK:_:");
        response[3] = c++;
        int len = 11;
        if(p_evt->params.rx_data.length < len)
        	len = p_evt->params.rx_data.length;
        memcpy(&response[5], p_evt->params.rx_data.p_data, len);
        len += 3;
        nusSend(response, len);
#endif // 0
    }
}
...
void BLEUart::send(const uint8_t *data, unsigned size)
{
	nusSend((uint8_t *)data, size);
}
